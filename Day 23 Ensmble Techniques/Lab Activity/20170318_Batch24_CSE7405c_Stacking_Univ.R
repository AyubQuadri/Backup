...
#convert to factors
data_combine <- apply(data_combine, 2, function(x) {as.factor(as.character(x))})
...
check1
acc1 = sum(diag(check1))/sum(check1); acc1
recall1 = sum(check1[2,2])/sum(check1[2,]); recall1
...
acc2 = sum(diag(check2))/sum(check2); acc2
recall2 = sum(check2[2,2])/sum(check2[2,]); recall2
...
acc3 = sum(diag(check3))/sum(check3); acc3
recall3 = sum(check3[2,2])/sum(check3[2,]); recall3
...
check4 <- table(train_pred_all_models$target,pred)
...
check5 <- table(testR$Personal.Loan, final_pred);check5
